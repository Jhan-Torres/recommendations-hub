<template>
  <div class="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-indigo-100 dark:from-gray-900 dark:via-purple-900/20 dark:to-gray-800">
    <!-- Header -->
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          <div class="flex items-center space-x-2 sm:space-x-3">
            <div class="p-1 sm:p-1.5 md:p-2 bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg">
              <Users class="h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 text-white" />
            </div>
            <h1 class="text-base sm:text-lg md:text-xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
              {{ t('community') }}
            </h1>
          </div>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <div class="flex flex-wrap gap-2 mb-6 sm:mb-8 justify-center">
        ...
        >
          <component :is="filter.icon" class="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" />
          {{ filter.label }}
          <span v-if="filter.value === 'saved' && savedPostsCount > 0" class="ml-1 sm:ml-2 px-1.5 py-0.5 bg-white/20 rounded-full text-xs font-bold">
            {{ savedPostsCount }}
          </span>
        </button>
      </div>
      ...
      <div v-if="selectedFilter === 'saved' && filteredPosts.length === 0" class="text-center py-12 sm:py-16">
        <div class="p-6 sm:p-8 bg-white/50 dark:bg-gray-800/50 rounded-xl border border-gray-200/50 dark:border-gray-700/50 max-w-md mx-auto">
          <Bookmark class="h-12 w-12 sm:h-16 sm:w-16 mx-auto text-gray-400 mb-4" />
          <h3 class="text-lg sm:text-xl font-medium text-gray-900 dark:text-white mb-2">{{ t('noSavedPosts') }}</h3>
          <p class="text-sm sm:text-base text-gray-500 dark:text-gray-400 mb-4">{{ t('startSavingPosts') }}</p>
          <button
      ...
      <div v-if="selectedFilter !== 'saved'" class="text-center mt-8 sm:mt-12">
        <button
          @click="loadMorePosts"
...
import { Users, Sparkles, ArrowLeft, RefreshCw, TrendingUp, Clock, Star, Grid, Bookmark } from 'lucide-vue-next'
...
const generateMockPosts = (count: number): CommunityPostType[] => {
  ...
  const authors = ['MovieBuff2024', 'SeriesAddict', 'AnimeExplorer', 'CinemaLover', 'StreamingPro', 'FilmCritic', 'ShowWatcher', 'EntertainmentGuru']
  const titles = [
    'Hidden Gem Alert!', 'Must Watch This Weekend', 'Underrated Masterpiece', 'Perfect for Binge Watching',
    'Emotional Rollercoaster', 'Mind-Blowing Plot Twist', 'Incredible Character Development', 'Stunning Visuals',
    'Heartwarming Story', 'Edge of Your Seat Thriller', 'Comedy Gold', 'Beautifully Crafted'
  ]
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    name: `Amazing ${categories[Math.floor(Math.random() * categories.length)]} ${Math.floor(Math.random() * 100)}`,
    genre: genres[Math.floor(Math.random() * genres.length)],
    rating: Math.floor(Math.random() * 5) + 1,
    author: authors[Math.floor(Math.random() * authors.length)],
    description: 'This is an incredible recommendation that you absolutely must check out. The storytelling is phenomenal and the characters are so well developed.',
