  ...
  >
    ...
    <div class="flex items-start justify-between mb-3 sm:mb-4">
      <div class="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
        <div class="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full flex items-center justify-center flex-shrink-0">
      ...
      >
        <Bookmark :class="post.isSaved ? 'fill-current' : ''" class="h-4 w-4 sm:h-5 sm:w-5" />
      </button>
    </div>

    <!-- Post Content -->
    <div class="mb-4">
      <h4 class="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">
        {{ post.title }}
      </h4>
      
      <div class="flex items-center space-x-2 sm:space-x-3 mb-3">
        <div :class="categoryStyles.bg" class="p-1.5 sm:p-2 rounded-lg flex-shrink-0">
          <component :is="categoryStyles.icon" :class="categoryStyles.text" class="h-3 w-3 sm:h-4 sm:w-4" />
        </div>
        <div class="min-w-0 flex-1">
      ...
      <p class="text-sm sm:text-base text-gray-600 dark:text-gray-300 line-clamp-2 mb-3">
        {{ post.description }}
      </p>
    ...
    <div class="flex items-center justify-between pt-3 border-t border-gray-200/50 dark:border-gray-700/50">
      <div class="flex items-center space-x-4">
        ...
        >
          <Heart :class="post.isLiked ? 'fill-current' : ''" class="h-3 w-3 sm:h-4 sm:w-4" />
    ...
    <div class="absolute inset-0 bg-purple-500/10 dark:bg-purple-400/10 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
      <div class="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm px-3 py-2 rounded-lg shadow-lg">
...
import { Film, Tv, Sparkles, Star, Heart, MessageCircle, Share, Bookmark, User, Eye } from 'lucide-vue-next'
...
const genreColor = computed(() => {
  ...
  return colors[props.post.genre as keyof typeof colors] || 'text-gray-600 dark:text-gray-400'
})
