<template>
  ...
  >
    ...
    >
      ...
      <div class="md:w-1/2 bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center p-8 relative">
        ...
        <div class="text-center text-white">
          <div :class="categoryStyles.bg" class="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl flex items-center justify-center mx-auto mb-6 bg-white/20 backdrop-blur-sm">
            <component :is="categoryStyles.icon" class="h-10 w-10 sm:h-12 sm:w-12 text-white" />
          </div>
          
          <h2 class="text-2xl sm:text-3xl font-bold mb-2">{{ post.name }}</h2>
          ...
          <div class="flex items-center justify-center space-x-1 mb-4">
            ...
            />
            <span class="ml-2 text-lg font-semibold">{{ post.rating }}/5</span>
          </div>
          
          <h3 class="text-xl font-bold mb-2">{{ post.title }}</h3>
          <p class="text-white/90 text-sm mb-4">{{ post.description }}</p>
          
          <p class="text-white/80 text-sm">
            {{ t('sharedBy') }} {{ post.author }} • {{ formatDate(post.createdAt) }}
          </p>
        </div>
      </div>
      ...
      <div class="md:w-1/2 flex flex-col max-h-[50vh] md:max-h-[90vh]">
        ...
        <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between flex-shrink-0">
          <div class="flex items-center space-x-4">
            ...
            <button class="flex items-center space-x-2 px-3 py-2 rounded-lg text-gray-500 dark:text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all duration-200 text-sm font-medium">
            ...
            >
              <Bookmark :class="isSaved ? 'fill-current' : ''" class="h-5 w-5" />
            </button>
          </div>
          
          <button class="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-all duration-200">
            <Share class="h-5 w-5" />
          </button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div class="flex space-x-3">
            ...
            <div class="flex-1">
              <div class="flex items-center space-x-2 mb-1">
                <span class="font-semibold text-sm text-gray-900 dark:text-white">{{ post.author }}</span>
          ...
          >
            <div class="w-8 h-8 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full flex items-center justify-center flex-shrink-0">
              <User class="h-4 w-4 text-white" />
            </div>
            <div class="flex-1">
              <div class="flex items-center space-x-2 mb-1">
              ...
              <div class="flex items-center space-x-4 mt-2">
                ...
                <button class="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                  {{ t('reply') }}
                </button>
              </div>
            </div>
          ...
          <div v-if="comments.length === 0" class="text-center py-8">
            <MessageCircle class="h-12 w-12 mx-auto text-gray-300 dark:text-gray-600 mb-3" />
            <p class="text-gray-500 dark:text-gray-400 text-sm">{{ t('noCommentsYet') }}</p>
            <p class="text-gray-400 dark:text-gray-500 text-xs">{{ t('beFirstToComment') }}</p>
          </div>
        </div>
          ...
          <div class="flex space-x-3">
            ...
            <div class="flex-1 flex space-x-2">
              <input
                ...
                class="flex-1 px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-full focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 transition-colors"
              />
              <button
                @click="addComment"
                :disabled="!newComment.trim()"
                class="px-4 py-2 bg-purple-500 text-white rounded-full text-sm font-medium hover:bg-purple-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                <Send class="h-4 w-4" />
              </button>
            </div>
...
import { 
  X, Star, Heart, MessageCircle, Share, Bookmark, User, Send,
  Film, Tv, Sparkles
...
const categoryStyles = computed(() => {
  if (!props.post) return { bg: '', text: '', icon: Film }
...
const formatDate = (date: Date) => {
  ...
  if (diffInHours < 168) return `${Math.floor(diffInHours / 24)}d ${t.value('ago')}`
...
watch(() => props.show, (newShow) => {
  if (newShow && props.post) {
    isLiked.value = props.post.isLiked
    isSaved.value = props.post.isSaved
