<template>
  ...
  >
    <div class="flex items-start justify-between mb-3 sm:mb-4">
      <div class="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
        <div :class="categoryStyles.bg" class="p-1.5 sm:p-2 rounded-lg flex-shrink-0">
          <component :is="categoryStyles.icon" :class="categoryStyles.text" class="h-4 w-4 sm:h-5 sm:w-5" />
        </div>
        <div class="min-w-0 flex-1">
          <h3 class="font-semibold text-sm sm:text-base text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors truncate">
            {{ recommendation.name }}
          </h3>
    ...
    <div class="absolute inset-0 bg-blue-500/10 dark:bg-blue-400/10 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
...
const genreColor = computed(() => {
  ...
  return colors[props.recommendation.genre as keyof typeof colors] || 'text-gray-600 dark:text-gray-400'
})
